...
## Cross-stratum continuation layer

The bifurcation ledger is now connected to a commit rule across adjacent
supports.
...
The initial/final polar supports

\[
T^\dagger T,\qquad TT^\dagger
\]

define the inherited corridor.

Lost and emergent sectors are represented by
...
This yields the continuity grammar

\[
\boxed{
\text{old witness}
\to
\text{canonical inherited witness}
+
\text{explicit lost mass}
+
\text{unwitnessed emergent support}.
}
\]

No new witness amplitude is generated by transport itself.

If endogenous formation of the emergent sector is required, only a simple
negative lowest compressed formation mode may seed it. Otherwise the
no-choice rule commits to \(\bot\).

Because the transition operators are partial isometries, the cross-stratum
transport layer is nonamplifying under arbitrary finite switching.
