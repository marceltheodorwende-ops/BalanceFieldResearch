...
## Cross-stratum continuation closure

Closed for the declared finite support category.

For orthogonal source/target support projectors,
...
Then \(T\) is the unique polar partial isometry and is equivariant under
ambient unitary conjugation.

For normalized witness \(\rho\),

\[
m_{\rm keep}
=
\operatorname{tr}(T^\dagger T\rho)
\]

is the exact inherited mass.
...
\rho_+^{\rm inh}
=
\frac{T\rho T^\dagger}{m_{\rm keep}}
\]

is the canonical target witness. If \(m_{\rm keep}=0\), witness continuity
terminates at \(\bot\).

On rank increase the emergent projector

\[
E=Q_+-TT^\dagger
\]

receives no inherited amplitude.

A required emergent seed exists only if the formation operator compressed
to \(E\) has a simple negative lowest mode.
...
\[
\|T_{n-1}\cdots T_0\|_2\le1,
\]

so cross-stratum witness transport cannot generate recursive amplification.

Current real carriers provide `1409/1409` self-continuation checks but no
observed first-step rank crossing. Seven controlled crossing cases validate
