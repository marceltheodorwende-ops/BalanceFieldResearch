...
from .cross_stratum import run_cross_stratum_audit, write_cross_stratum_report
...
def cross_stratum_audit(args):
    result=run_cross_stratum_audit(
        limit_per_timeseries_carrier=args.limit,
        include_full_network=True,
    )
    files=write_cross_stratum_report(
        result,
        args.out,
    )
    compact={
        "heldout_metrics_evaluated":
            result["heldout_metrics_evaluated"],
        "real_states_total":result["real_states_total"],
        "real_observed_rank_crossings":
            result["real_observed_rank_crossings"],
        "real_identity_continuity_passed":
            result["real_identity_continuity_passed"],
        "synthetic_crossing_cases":
            result["synthetic_crossing_cases"],
        "synthetic_crossing_passed":
            result["synthetic_crossing_passed"],
        "unitary_equivariance_passed":
            result["unitary_equivariance_passed"],
        "identity_continuity_passed":
            result["identity_continuity_passed"],
        "cross_stratum_chain_stability_passed":
            result["cross_stratum_chain_stability_passed"],
        "domains":result["domains"],
        "files":files,
    }
    print(json.dumps(compact,indent=2,default=str))
...
def main():
    ...
    cs=sub.add_parser("cross-stratum-audit",help="audit canonical polar support transport, witness continuity, and no-choice cross-stratum transitions")
    cs.add_argument("--limit",type=int,default=None,help="optional per-time-series-carrier smoke-test limit; network remains complete")
    cs.add_argument("--out",type=str,default="outputs/cross_stratum")
    cs.set_defaults(func=cross_stratum_audit)
