...
def admissible_perturbation_certificate(
    base: np.ndarray,
    ...
    *,
    ...
    """
    Perturbation audit with the correct stability distinction.

    Small operator norm alone does not preserve exact unit-circle persistence or
    power boundedness. The perturbed operator must independently remain inside
    the finite power-bounded class (or a stronger constrained perturbation class
    must be specified).
    """
    a=np.asarray(base,dtype=complex)
    b=np.asarray(perturbed,dtype=complex)
    if a.shape!=b.shape:
        raise ValueError("base and perturbed operators must have same shape")

    base_cert=recursive_stability_certificate(
        a,
        unit_tol=unit_tol,
    ...
    pert_cert=recursive_stability_certificate(
        b,
        unit_tol=unit_tol,
        grouping_tol=grouping_tol,
        svd_rtol=svd_rtol,
    ...
    delta=float(np.linalg.norm(b-a,2))
    return {
        "operator_norm_perturbation":delta,
        "base_power_bounded":base_cert.spectral_criterion_satisfied,
        "perturbed_power_bounded":
            pert_cert.spectral_criterion_satisfied,
        "base_peripheral_count":base_cert.peripheral_count,
        "perturbed_peripheral_count":pert_cert.peripheral_count,
        "admissible_stability_preserved":bool(
            base_cert.spectral_criterion_satisfied
            and pert_cert.spectral_criterion_satisfied
        ),
        "smallness_alone_sufficient":False,
        "note":(
            "Cluster separation can be perturbatively stable, but exact "
            "unit-circle persistence/power boundedness requires the perturbed "
            "operator to remain in the admissible class."
        ),
    }
...
def run_master_stability_audit(
    ...
    *,
    ...
    outward=np.diag([1.0+1e-4,0.5]).astype(complex)
    tangential=np.diag([np.exp(1e-4j),0.5]).astype(complex)
    ...
    controls_pass=bool(
        ...
        and not controls["small_outward_perturbation"][
            "admissible_stability_preserved"
        ]
        and controls["tangential_unit_circle_perturbation"][
            "admissible_stability_preserved"
        ]
    )
...
def write_stability_audit(
    ...
    lines += [
        ...
        f"- small outward unit-circle perturbation rejected: "
        f"`{not result['controls']['small_outward_perturbation']['admissible_stability_preserved']}`",
        f"- tangential unit-circle perturbation remains admissible: "
        f"`{result['controls']['tangential_unit_circle_perturbation']['admissible_stability_preserved']}`",
        "",
