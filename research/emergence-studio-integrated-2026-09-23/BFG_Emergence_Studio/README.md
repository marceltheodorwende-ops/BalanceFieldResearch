...
- `source-math-audit`: audit the integrated BalanceFieldResearch exact/conditional mathematics
...
## BalanceFieldResearch mathematical merge

The current master now incorporates the mathematical additions from the
`marceltheodorwende-ops/BalanceFieldResearch` research line in a
claim-sensitive way.
...
```bash
python -m bfg_studio source-math-audit \
  --out outputs/source_math
```

See `GITHUB_BALANCEFIELDRESEARCH_INTEGRATION.md` and
`GITHUB_SOURCE_INTEGRATION_MANIFEST.json`.
