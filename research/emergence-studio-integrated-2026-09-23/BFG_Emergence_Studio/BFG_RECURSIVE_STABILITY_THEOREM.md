...
If the peripheral spectrum is isolated by a positive gap and remains semisimple,
then sufficiently small operator perturbations preserve the separation between
the peripheral and stable spectral sectors.

The precise perturbation radius depends on spectral conditioning. Therefore the
master software records the stable gap but does not claim one universal
carrier-independent perturbation radius.

For normal recursive operators, standard spectral perturbation bounds make this
particularly direct. For nonnormal operators, conditioning must be reported
rather than hidden.
...
The BalanceFieldResearch nonnormal stage strengthens the numerical
