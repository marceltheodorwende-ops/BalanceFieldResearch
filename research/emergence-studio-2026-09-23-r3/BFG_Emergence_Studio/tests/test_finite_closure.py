...
from bfg_studio import (
    simulate,
    make_random_seed_state,
    ReferenceGramCarrier,
    ReducedRunRecord,
    ReducedClosureState,
...
def test_unified_simulate_dispatches_to_finite_master_without_carrier():
    seed=make_random_seed_state(dim=5,persistent_rank=3,seed=7)
    run=simulate(seed,carrier=None,steps=1,runtime="auto")
    assert isinstance(run,ReducedRunRecord)
    assert run.summary()["runtime_profile"]=="finite_reduced_closure"

def test_unified_simulate_keeps_explicit_carrier_lane_compatible():
    seed=make_random_seed_state(dim=5,persistent_rank=3,seed=7)
    run=simulate(
        seed,
        ReferenceGramCarrier(),
    ...
    assert run.summary()["steps_attempted"]==1


def test_internal_numerical_admissibility_boundary_maps_to_bottom():
    state=ReducedClosureState(
        rho=np.array([[1.0]],complex),
        K=np.array([[-1.0]],complex),
        Y=np.array([[1e-12]],complex),
        R=np.array([[1.0]],complex),
        generation=3,
        metadata={"master_profile":"finite_reduced_closure"},
    )
    nxt,step=reduced_closure_step(state)
    assert nxt.bottom
    assert step.terminal
    assert not step.success
    assert "numerical admissibility boundary" in step.reason
    assert step.diagnostics["numerical_boundary"] is True


def test_candidate_successor_below_numerical_floor_maps_to_bottom():
    # Input Y is just above the declared positive-definite floor, while the
    # analytic successor load is O(Y^2) and therefore falls below it.
    state=ReducedClosureState(
        rho=np.array([[1.0]],complex),
        K=np.array([[-1.0]],complex),
        Y=np.array([[2e-5]],complex),
        R=np.array([[1.0]],complex),
        generation=0,
    )
    nxt,step=reduced_closure_step(state)
    ...
    assert step.terminal
    assert not step.success
    assert "successor admissibility boundary" in step.reason
    assert step.diagnostics["numerical_boundary"] is True
