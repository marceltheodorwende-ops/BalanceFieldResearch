...
@dataclass
class ReducedRunRecord:
    states: list[ReducedClosureState]
    steps: list[ReducedClosureStep]
    runtime_profile: str = "finite_reduced_closure"

    def summary(self) -> dict[str,Any]:
        successful=sum(int(s.success) for s in self.steps)
        terminal=bool(self.states[-1].bottom) if self.states else True
        reason=self.states[-1].reason if terminal and self.states else None
        return {
            ...
            "final_generation":(
                self.states[-1].generation if self.states else -1
            ),
        }


def bfg_state_to_reduced(
    state:BFGState,
) -> ReducedClosureState:
    """
    Canonical adapter from the legacy vector-state carrier envelope into the
    active reduced finite master state.

    The state vector D induces the rank-one density rho = |D><D| / <D|D>.
    K, Y and R_C are transported directly.
    """
    if state.terminal:
        return bottom_state(
            state.generation,
            state.terminal_reason or "legacy state already terminal",
            metadata={"adapter_source":state.name},
        )
    d=np.asarray(state.D,dtype=complex).reshape(-1)
    norm=float(np.real(np.vdot(d,d)))
    if not np.isfinite(norm) or norm<=0:
        raise ValueError("legacy D cannot induce a nonzero rank-one density")
    rho=np.outer(d,d.conj())/norm
    return ReducedClosureState(
        ...
        metadata={
            "adapter_source":state.name,
            "adapter":"BFGState -> ReducedClosureState",
            **dict(state.metadata),
        },
    )


def run_reduced_runtime(
    initial:ReducedClosureState|BFGState,
    steps:int=10,
    *,
    tau:float=1.0,
    ...
    """
    First-class master runtime entry point.

    Accepts either a reduced state directly or the legacy BFGState envelope,
    which is converted through bfg_state_to_reduced.
    """
    state=(
        bfg_state_to_reduced(initial)
        if isinstance(initial,BFGState)
        else initial
    )
    if not isinstance(state,ReducedClosureState):
        raise TypeError(
            "finite master runtime requires ReducedClosureState or BFGState"
        )
    raw=iterate_reduced_closure(
        state,
        steps,
        tau=tau,
        policy=policy,
    )
    return ReducedRunRecord(
        states=raw["states"],
        steps=raw["steps"],
    )
...
def reduced_closure_step(
    ...
    *,
    ...
    try:
        ...
        # The mathematical master map is total after adjoining bottom. External
        # invalid inputs remain programmer/data errors, but an internally
        # generated master successor that reaches the finite-precision
        # admissibility floor is transported to bottom instead of escaping the
        # runtime as an exception.
        if state.metadata.get("master_profile")=="finite_reduced_closure":
            nxt=bottom_state(
                state.generation+1,
                f"numerical admissibility boundary: {exc}",
                metadata={
                    "parent_generation":state.generation,
                    "master_profile":"finite_reduced_closure",
                    "numerical_boundary":True,
                },
            )
    ...
    try:
        successor_validation=_validate_reduced_state(
            next_state,
            policy,
        )
    except ValueError as exc:
        # A successor can be mathematically positive while falling below the
        # declared floating-point admissibility floor. Preserve totality by
        # committing that transition to bottom rather than leaking an exception.
        nxt=bottom_state(
            state.generation+1,
            f"numerical successor admissibility boundary: {exc}",
            metadata={
                "parent_generation":state.generation,
                "master_profile":"finite_reduced_closure",
                "numerical_boundary":True,
                "candidate_unitarity_residual":unitary_res,
                "candidate_gram_rebuild_residual":gram_res,
            },
        )
        return nxt,ReducedClosureStep(
            generation=state.generation,
            ...
            diagnostics={
                ...
                "numerical_boundary":True,
                "successor_validation_error":str(exc),
                "candidate_unitarity_residual":unitary_res,
                "candidate_gram_rebuild_residual":gram_res,
